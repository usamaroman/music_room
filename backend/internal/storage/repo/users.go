...
func (repo *Users) Create(ctx context.Context, user *dbo.User) error {
	q := `insert into users (email, nickname, password) values ($1, $2, $3)`

	exec, err := repo.qb.Pool().Exec(ctx, q, user.Email, user.Nickname, user.Password)
	if err != nil {
	...
	repo.log.Info("execution", zap.Int64("rows affected", exec.RowsAffected()))
